...
def suborder(output,ranks):
    if len(output) > 1:
        for i in output:
            if i != output[0]:
                higher = []
                lower = []
                if ranks[i] > ranks[output[0]]:
                    higher.append(i)
                else:
                    lower.append(i)
                    higher.append(output[0])
                    higher = suborder(higher,ranks)
                    lower = suborder(lower,ranks)
                    output = higher + lower
    return output
