...
class Zombie(poc_grid.Grid):
    ...
    def compute_distance_field(self, entity_type):
        ...
        def bfs(lst):
            """
            Breath First Search help function
            """
            boundary = poc_queue.Queue()
            for each in lst:
                visited.set_full(each[0], each[1])
                dist_field[each[0]][each[1]] = 0
                boundary.enqueue(each)
            while len(boundary) > 0:
                cur_cell = boundary.dequeue()
                four_neighbors = \
                    poc_grid.Grid.four_neighbors(self, cur_cell[0], cur_cell[1])
                for each_neighbor in four_neighbors:
                    if visited.is_empty(
                        each_neighbor[0], each_neighbor[1]) and \
                        poc_grid.Grid.is_empty(
                        self, each_neighbor[0], each_neighbor[1]):
                        visited.set_full(each_neighbor[0], each_neighbor[1])
                        if dist_field[cur_cell[0]]\
                            [cur_cell[1]]+1 < dist_field[each_neighbor[0]]\
                            [each_neighbor[1]]:
                            dist_field[each_neighbor[0]][each_neighbor[1]] = \
                                dist_field[cur_cell[0]][cur_cell[1]]+1
        ...
        if entity_type == HUMAN:
            return bfs(self._human_list)
        elif entity_type == ZOMBIE:
            return bfs(self._zombie_list)
    ...
    def move_humans(self, zombie_distance):
        ...
        for idx in range(len(self._human_list)):
            eight_neighbor_human = \
                poc_grid.Grid.eight_neighbors(
                self, self._human_list[idx][0], self._human_list[idx][1])
            max_distance = \
                zombie_distance[self._human_list[idx][0]]\
                [self._human_list[idx][1]]
            max_pos = (self._human_list[idx][0], self._human_list[idx][1])
            for eight_neighbor in eight_neighbor_human:
                if zombie_distance[eight_neighbor[0]]\
                    [eight_neighbor[1]] > max_distance:
                    max_distance = \
                        zombie_distance[eight_neighbor[0]][eight_neighbor[1]]
    ...
    def move_zombies(self, human_distance):
        ...
        for idx in range(len(self._zombie_list)):
            four_neighbor_zombie = \
                poc_grid.Grid.four_neighbors(
                self, self._zombie_list[idx][0], self._zombie_list[idx][1])
            min_distance = \
                human_distance[self._zombie_list[idx][0]]\
                [self._zombie_list[idx][1]]
            min_pos = (self._zombie_list[idx][0], self._zombie_list[idx][1])
            for four_neighbor in four_neighbor_zombie:
                if human_distance[four_neighbor[0]]\
                    [four_neighbor[1]] < min_distance:
                    min_distance = \
                        human_distance[four_neighbor[0]][four_neighbor[1]]
