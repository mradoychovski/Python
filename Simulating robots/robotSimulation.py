...
# For Python 2.7:
# from verify_movement27 import testRobotMovement
...
def runSimulation(num_robots, speed, width, height, min_coverage, num_trials,
    ...
    robot_type: class of robot to be instantiated (e.g. StandardRobot or
                RandomWalkRobot)
    """
    def buildRobotCol(num_robots):
        robotCol = []
        for i in range(num_robots):
            robotCol.append(robot_type(room, speed))
        return robotCol

    def updateRobotPosAndCln(robotCol):
        for robot in robotCol:
    ...
    for trial in range(num_trials):
        ...
        robotCollection = buildRobotCol(num_robots)
        if visualize:
            anim.update(room, robotCollection)
        while (room.getNumCleanedTiles()/float(room.getNumTiles())) < min_coverage:
            updateRobotPosAndCln(robotCollection)
            totalTimeSteps += 1
            if visualize:
                anim.update(room, robotCollection)
        if visualize:
            anim.done()
    return totalTimeSteps / num_trials
    #raise NotImplementedError
